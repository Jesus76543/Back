    ...
    try {
        ...
        for (const { productId, quantity } of products) {
            ...
            if (!product || !product.status || product.Quantity < quantity) {
                res.status(400).json({
                    message: `Producto no disponible o stock insuficiente: ${product?.name || productId}`,
                });
                return;
            }
            // Actualizar stock
...
export const updateOrderStatus = async (req, res) => {
    try {
        ...
        const updatedOrder = await Orden.findByIdAndUpdate(orderId, { status }, { new: true });
        if (!updatedOrder) {
            res.status(404).json({ message: "Orden no encontrada" });
            return;
        }
...
export const cancelOrder = async (req, res) => {
    try {
        ...
        const cancelledOrder = await Orden.findByIdAndUpdate(orderId, { status: false, deleteAt: new Date() }, { new: true });
        res.json({
