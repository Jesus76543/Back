...
export function validateEnvironment() {
    ...
    if (process.env.JWT_SECRET) {
        ...
        if (weakSecrets.some((weak) => process.env.JWT_SECRET?.toLowerCase().includes(weak))) {
            errors.push("JWT_SECRET appears to be using a default or weak value. Please generate a strong secret.");
        }
    }
    ...
    if (process.env.BCRYPT_SALT_ROUNDS) {
        ...
        if (isNaN(saltRounds) || saltRounds < 10 || saltRounds > 15) {
            warnings.push("BCRYPT_SALT_ROUNDS should be between 10-15 for optimal security");
        }
    }
    // Log warnings
    ...
    if (errors.length > 0) {
        ...
        console.error("   3. Generate JWT_SECRET with: node -p \"require('crypto').randomBytes(64).toString('hex')\"");
        console.error("");
        throw new EnvironmentError(`Missing ${errors.length} critical environment variable(s)`);
    }
